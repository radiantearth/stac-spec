...
| item    | URL to a STAC [Item](../item-spec/item-spec.md). All items linked from a collection MUST refer back to its collection with the [`collection` relation type](../item-spec/item-spec.md#relation-types). Links with a `rel` of item have a default `type` of `application/geo+json`. This `rel` can also be used with '[unlocated items](../best-practices.md#unlocated-items)' by explicitly setting the `type` to `application/json`. |
