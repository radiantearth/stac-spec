...
| id           | string            | **REQUIRED.** Identifier for the collection that is unique across the provider. The use of URI reserved characters such as `:` or `/` is discouraged since this will result in [percented encoded](https://tools.ietf.org/html/rfc3986#section-2) STAC API endpoints. |
